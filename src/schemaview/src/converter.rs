...
/// async fetch, etc.), prefer switching back to the upstream `curies` crate
/// rather than expanding this module.
#[derive(Debug, Clone)]
...
impl Converter {
    ...
    pub fn add_record(&mut self, record: Record) -> Result<(), ConverterError> {
        ...
        let uri_prefixes: Vec<String> = record.all_uri_prefixes().map(|p| p.to_string()).collect();
    ...
    pub fn compress(&self, uri: &str) -> Result<String, ConverterError> {
        ...
        let (record, prefix_len) = best.ok_or_else(|| ConverterError::NotFound(uri.to_string()))?;
