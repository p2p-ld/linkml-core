...
type ViewIter<'a, T> = std::iter::Map<std::slice::Iter<'a, Box<T>>, fn(&'a Box<T>) -> &'a T>;
...
fn debox_mut<'a, T>(b: &'a mut Box<T>) -> &'a mut T {
    &mut **b
}
...
fn as_pair<'a, K, V>((k, v): (&'a K, &'a Box<V>)) -> (&'a K, &'a V) {
    (k, &**v)
}
...
impl<'a, K: Eq + Hash, V> MapViewMut<'a, K, V> {
    ...
    pub fn get_mut(&mut self, k: &K) -> Option<&mut V> {
        self.inner.get_mut(k).map(debox_mut)
    }
...
type MapIterMut<'a, K, V> = Map<
    std::collections::hash_map::IterMut<'a, K, Box<V>>,
    fn((&'a K, &'a mut Box<V>)) -> (&'a K, &'a mut V),
>;

fn as_pair_mut<'a, K, V>((k, v): (&'a K, &'a mut Box<V>)) -> (&'a K, &'a mut V) {
    (k, &mut **v)
}
...
impl<'a, K: Eq + Hash, V> MapRef<'a, K, V> for &'a HashMap<K, V> {
    ...
    type Iter<'b>
        = std::collections::hash_map::Iter<'b, K, V>
    where
        Self: 'b,
        K: 'b,
        V: 'b;
...
impl<'a, K: std::hash::Hash + Eq, V> MapRef<'a, K, V> for MapView<'a, K, V> {
    ...
    fn iter(&self) -> Self::Iter<'_> {
        fn as_pair<'b, K, V>((k, v): (&'b K, &'b Box<V>)) -> (&'b K, &'b V) {
            (k, &**v)
        }
        self.inner.iter().map(as_pair)
    }
...
impl<'a, K: Eq + std::hash::Hash, V> MapRef<'a, K, V> for MapAny<'a, K, V> {
    type Iter<'b>
